...
project=tinnitus  			# The name of your project in /neuro/data/sinuhe
trans_conditions=( 'tinmeg1b' ) 		# Name(s) of condition(s) on which head position correction should be applied
...
#		confiles=$(find ./*$condition* 2> /dev/null)
		if [[ ! -z $confiles ]]; then
			run_trans="yes"
			echo "Found ${#confiles[@]} files to transform for condition '$condition' (excluding split-files): ${confiles[@]}"
			break
		fi
	done	
	
	echo "Run trans = $run_trans"
	if [[ $run_trans == "yes" ]]; then
		...
		if [[ "$trans_option" == "initial" ]]; then
			...
			do
				...
				ipython $script_path/avg_headpos2.py $trans_option $condition
			done
...
echo /$quat_folder/$quat_fname
					...
					if [[ ! -f ./$quat_folder/$quat_fname ]]; then
						...
						# Run maxfilter
						/neuro/bin/util/maxfilter -f ./${fname} -o ./$quat_fname $ds -headpos -hp ./$headpos_folder/$pos_fname -autobad $autobad -badlimit $badlimit -skip 0 31 | tee -a ./log/${quat_fname:0:$length}.log
#					echo "Would run initial MaxF here"
				...
				ipython $fun_path/avg_headpos2.py $trans_option $condition $(pwd) $trans_type 
...
############################################################################################################################################################################################################
		...
		/neuro/bin/util/maxfilter -f ${filename} -o ${output_file} $force $tsss $ds -corr $correlation $movecomp $trans -autobad $autobad -badlimit $badlimit -cal $cal -ctc $ctc -v $headpos $linefreq -skip 0 31  | tee -a ./log/${filename:0:$length}${tsss_string}${movecomp_string}${trans_string}${linefreq_string}${ds_string}.log
